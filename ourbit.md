API Library
We provide demos for developers in five languages: Python, DotNET, Java, JavaScript, and Go, offering users direct reference to call the API through the demos. Currently, we support all spot trading interfaces.

https://github.com/ourbitdevelop/ourbit-api-demo

 Any problem please submit feedback
Postman Collections
...
For more information please refer to this page: OURBIT API Postman

OURBIT Broker Introduction
OURBIT is committed to building crypto infrastructure, with API broker partners that provide valuable services being an essential part of the OURBIT ecosystem. To reward the partners, OURBIT now provides privileges for OURBIT brokers, including trading rebates and marketing support.

Broker Modes Supported by OURBIT
...
This includes wallet platforms, market data platforms, aggregation trading platforms, stockbrokers, as well as stock and securities trading platforms, etc., all of which have their own independent users. OURBIT can provide order matching systems, account management systems, settlement systems, as well as main and sub-account systems, etc. Independent brokers can share the trading fluidity and depth over the OURBIT platform and receive profit shares from fees.
...
If there is no valid subscription, it will disconnect in 30 seconds.
If the subscription is successful, and there is no traffic in 60 seconds, it will automatically disconnect.
To ensure a stable connection, a Pong reply is required upon receiving the Ping message sent from the server.
...
HTTP 5XX return codes are used for internal errors; the issue is on OURBIT's side. It is important to NOT treat this as a failure operation; the execution status is UNKNOWN and could have been a success.
...
curl command:
    ...
    (HMAC SHA256)
    $ curl -H "X-OURBIT-APIKEY: mx0aBYs33eIilxBWC5" -X POST 'https://api.ourbit.com/api/v3/order' -d 'symbol=BTCUSDT&side=BUY&type=LIMIT&quantity=1&price=11&recvWindow=5000&timestamp=1644489390087&signature=323c96ab85a745712e95e63cad28903dd8292e4a905e99c4ee3932023843a117'
...
curl command:
    ...
    (HMAC SHA256)
    $ curl -H "X-OURBIT-APIKEY: mx0aBYs33eIilxBWC5" -X POST 'https://api.ourbit.com/api/v3/order' -d 'symbol=BTCUSDT&side=BUY&type=LIMIT&quantity=1&price=11&recvWindow=5000&timestamp=1644489390087&signature=fd3e4e8543c5188531eb7279d68ae7d26a573d0fc5ab0d18eb692451654d837a'
...
curl command:
    ...
    (HMAC SHA256)
    $ curl -H "X-OURBIT-APIKEY: mx0aBYs33eIilxBWC5" -X POST 'https://api.ourbit.com/api/v3/order?symbol=BTCUSDT&side=BUY&type=LIMIT' -d 'quantity=1&price=11&recvWindow=5000&timestamp=1644489390087&signature=d1a676610ceb39174c8039b3f548357994b2a34139a8addd33baadba65684592'
...
Method	Example
No parameter	curl -X GET "https://api.ourbit.com/api/v3/exchangeInfo"
symbol	curl -X GET "https://api.ourbit.com/api/v3/exchangeInfo?symbol=OBUSDT"
symbols	curl -X GET "https://api.ourbit.com/api/v3/exchangeInfo?symbols=OBUSDT,BTCUSDT"
Response:
...

{
    "subAccounts":[
        {
            "subAccount":"ourbit666",
            "isFreeze":false,
            "createTime":1544433328000,
            "uid": "49910511"
        },
        {
            "subAccount":"ourbit888",
            "isFreeze":false,
            "createTime":1544433328000,
            "uid": "91921059"
        }
...
POST /api/v3/order?symbol=OBUSDT&side=BUY&type=LIMIT&quantity=50&price=0.1&timestamp={{timestamp}}&signature={{signature}}
Response
...
Name	Type	Mandatory	Description
symbol	string	YES	maximum input 5 symbols,separated by ",". e.g. "BTCUSDT,OBUSDT,ADAUSDT"
recvWindow	long	NO	
timestamp	long	YES	
Response:
...
Enable OB Deduct
Enable or disable OB deduct for spot commission fee

Request

post api/v3/obDeduct/enable
Response
...
Name	Type	Mandatory	Description
obDeductEnable	boolean	yes	true:enable,false:disable
...
Name	Type	Description
obDeductEnable	boolean	true:enable,false:disable
For Futures:Enjoy 10% off trading fees when you transfer OB into your futures account.
Query OB Deduct Status
Request

get api/v3/obDeduct/enable
Response

{
  "data":{
    "obDeductEnable":false
  },
  "code":0,
  "msg":"success",
...
post /api/v3/capital/withdraw/apply?coin=EOS&address=zzqqqqqqqqqq&amount=10&network=EOS&memo=OB10086&timestamp={{timestamp}}&signature={{signature}}
Response
...
timestamp	string	YES	timestamp
signature	string	YES	signature
default return the records of the last 7 days.
Ensure that the default timestamp of 'startTime' and 'endTime' does not exceed 7 days.
...
status	status
timestamp	timestamp
Get Assets That Can Be Converted Into OB
Request
...
Name	Description
convertOb	OB amount（Deducted commission fee）
...
Name	Description
totalConvert	Convert OB amount(Deducted commission fee)
...
The base endpoint is: wss://wbs.ourbit.com/ws.

A single connection to wbs.ourbit.com is only valid for 24 hours; expect to be disconnected at the 24 hour mark.

All symbols are Uppercase
eg:spot@public.deals.v3.api@<symbol>
...
Though spot@public.increase.depth.v3.api@ to get full amount of depth information, save the current version.
...
Through https://api.ourbit.com/api/v3/depth?symbol=OBBTC&limit=1000 get the latest 1000 depth snapshots.
...
The base API endpoint is: https://api.ourbit.com
...
websocket baseurl: wss://wbs.ourbit.com/ws

User Data Streams are accessed at /ws?listenKey=listenKey
eg:wss://wbs.ourbit.com/ws?listenKey=pqia91ma19a5s61cv6a81va65sd099v8a65a1a5s61cv6a81va65sdf19v8a65a1
