...
- lpaca Broker* Total value of cash + holding positions (Equivalent to the equity field)
 ...
 | `multiplier` | string`<number>` | Buying power (BP) multiplier that represents account margin classificationValid values:
...
- 4 (PDT account with 4x intraday BP and 2x reg T overnight BP)
 ...
 | `buying_power` | string`<number>` | Current available $ buying power; If multiplier = 4, this is your daytrade buying power which is calculated as (last_equity - (last) maintenance_margin)_ 4; If multiplier = 2, buying_power = max(equity – initial_margin,0)_ 2; If multiplier = 1, buying_power = cash
...
statusdescription
 ...
 | `REJECTED` | The account application has been rejected.Updated about 2 months ago Paper TradingCrypto Spot Trading- Ask AI
