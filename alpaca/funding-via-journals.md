...
- Upon creation, the journal will be created in a `queued` state.
- Then, the journal will be `sent_to_clearing` meaning that the request has been submitted to our books and records system.
- Lastly, if there are no issues the journal will be `executed`, meaning that the cash or securities have been successfully moved into the receiving account.
...
StatusDescription
 ...
 | `pending` | The journal is pending to be processed as it requires manual approval from Alpaca operations, for example, this can be caused by hitting the journal limits.
...
Journal statuses flowchartUpdated 5 months ago Funding AccountsFunding Wallets- Ask AI
